...
- ✅ **控制键发送**：支持 Ctrl+C、方向键等组合键，可打断卡死命令（`top`/`ping`/`tail -f` 等）
...
| 命令与输出 | 5 | `send_command` `send_key` `get_last_output` `get_all_output` `clear_output` |
...
v1.0.4 新增，用于向终端发送非文本控制指令，解决交互式场景卡死问题。

### 场景示例

```
# AI 误执行了 top，命令不会自动返回
send_command("top", wait_ms=3000)   → 3秒后超时返回

# 立即发送 Ctrl+C 打断
send_key("ctrl+c")                  → 立即返回 "OK"
...
| 分类 | 值 | 效果 |
|------|---|------|
| **控制字符** | `ctrl+c` ~ `ctrl+z` | 发送对应 ASCII 控制字符 (`\x01`~`\x1a`) |
| **导航** | `up` `down` `left` `right` | 方向键 |
| **功能键** | `home` `end` `pgup` `pgdn` `insert` `delete` | 功能键 |
| **特殊键** | `enter` `tab` `esc` `backspace` `space` | 回车/制表/ESC/退格/空格 |
| **文本** | `text:exit` `text:y` `text:自定义` | 直接写入文本（不需加回车） |

> ⚠️ PTY 和 SSH2 双引擎一致支持，不等待返回结果，立即响应。
